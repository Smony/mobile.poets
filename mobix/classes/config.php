...
class database {
  ...
  public function insert($request, $data, $column){
    ...
    $query = mysql_query("SELECT last_insert_id() as `{$column}`", $this->conn);
    $array = $this->result2array($query);
    return isset($array[0][$column]) ? $array[0][$column] : 0;
  }
